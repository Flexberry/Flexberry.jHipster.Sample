...
describe('Location e2e test', () => {
  ...
  describe('new Location page', () => {
    ...
    it('should create an instance of Location', () => {
      cy.get(`[data-cy="streetAddress"]`).type('systems').should('have.value', 'systems');

      cy.get(`[data-cy="postalCode"]`).type('Региональный улица логистический').should('have.value', 'Региональный улица логистический');

      cy.get(`[data-cy="city"]`).type('Lake Sierraland').should('have.value', 'Lake Sierraland');

      cy.get(`[data-cy="stateProvince"]`).type('payment e-commerce').should('have.value', 'payment e-commerce');
