...
describe('Employee e2e test', () => {
  ...
  describe('new Employee page', () => {
    ...
    it('should create an instance of Employee', () => {
      ...
      cy.get(`[data-cy="lastName"]`).type('Блохина').should('have.value', 'Блохина');

      cy.get(`[data-cy="email"]`).type('Dell_@yandex.ru').should('have.value', 'Dell_@yandex.ru');

      cy.get(`[data-cy="phoneNumber"]`).type('серый').should('have.value', 'серый');

      cy.get(`[data-cy="hireDate"]`).type('2023-05-30T00:31').blur().should('have.value', '2023-05-30T00:31');

      cy.get(`[data-cy="salary"]`).type('90786').should('have.value', '90786');

      cy.get(`[data-cy="commissionPct"]`).type('80256').should('have.value', '80256');
